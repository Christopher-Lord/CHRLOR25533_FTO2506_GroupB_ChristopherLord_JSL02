...
function addTask() {
  ...
  do {
    ...
    task.status = task.status.toLowerCase(); //Making the 'status' input lowercase for consistency
...
while (confirm("Would you like to add a new task?")) {
  console.clear(); //Clears the console at the start of every task entry, not necessary, but makes the console neater
